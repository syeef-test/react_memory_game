...
export const getHighScore = async (req, res, next) => {
  try {
    const response = await HighScore.find().sort({ score: -1 }).limit(10);
...
export const getSingleHighScore = async (req, res, next) => {
  try {
    const response = await HighScore.find().sort({ score: -1 }).limit(1);
